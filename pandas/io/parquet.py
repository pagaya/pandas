...
def is_s3_path(path):
    index = path.index('/')
    return path[: index - 1] in ['s3','s3a','s3n']

def split_s3_path(path):
    index = path.index('/')
    path = path[index + 2:] # remove s3/s3a/s3n prefix
    index = path.index('/')
    return path[:index], path[index + 1:]

def concurrent_read(path, **kwargs):
...
def read_parquet(path, **kwargs):
    if is_s3_path(path):
        path = concurrent_read(path, **kwargs)
    return org_read_parquet(path, **kwargs)

def org_read_parquet(path, engine: str = "auto", columns=None, **kwargs):
    """
